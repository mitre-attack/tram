...
class RestService:
    ...
    async def false_negative(self, criteria=None):
        sentence_dict = await self.dao.get(constant.DB_TABLE_REPORT_SENTENCES, dict(uid=criteria['sentence_id']))
        sentence_to_strip = sentence_dict[0]['text']
        sentence_to_insert = self.web_svc.remove_html_markup_and_found(sentence_to_strip)
        await self.dao.insert(constant.DB_TABLE_FALSE_NEGATIVES, dict(sentence_id=sentence_dict[0]['uid'], 
                                                                      uid=criteria['attack_uid'],
                                                                      false_negative=sentence_to_insert))
        return dict(status=constant.STATUS_INSERTED)

    async def set_status(self, criteria=None):
        report_dict = await self.dao.get(constant.DB_TABLE_REPORTS, dict(title=criteria['file_name']))
        await self.dao.update(constant.DB_TABLE_REPORTS, 'uid', report_dict[0]['uid'], dict(current_status=criteria['set_status']))
        return dict(status=constant.STATUS_REPORT_UPDATED + criteria['set_status'])

    async def delete_report(self, criteria=None):
        await self.dao.delete(constant.DB_TABLE_REPORTS, dict(uid=criteria['report_id']))
        await self.dao.delete(constant.DB_TABLE_REPORT_SENTENCES, dict(report_uid=criteria['report_id']))
        await self.dao.delete(constant.DB_TABLE_REPORT_SENTENCE_HITS, dict(report_uid=criteria['report_id']))

    async def remove_sentences(self, criteria=None):
        if not criteria['sentence_id']:
            return dict(status=constant.STATUS_ENTER_NUMBER)
        else:
            true_positives = await self.dao.get(constant.DB_TABLE_TRUE_POSITIVES, dict(sentence_id=criteria['sentence_id']))
            false_positives = await self.dao.get(constant.DB_TABLE_FALSE_POSITIVES, dict(sentence_id=criteria['sentence_id']))
            false_negatives = await self.dao.get(constant.DB_TABLE_FALSE_NEGATIVES, dict(sentence_id=criteria['sentence_id']))
        if not true_positives and not false_positives and not false_negatives:
            return dict(status=constant.STATUS_NO_ENTRY_SENTENCE_ID + criteria['sentence_id'])
        else:
            await self.dao.delete(constant.DB_TABLE_TRUE_POSITIVES, dict(sentence_id=criteria['sentence_id']))
            await self.dao.delete(constant.DB_TABLE_FALSE_POSITIVES, dict(sentence_id=criteria['sentence_id']))
            await self.dao.delete(constant.DB_TABLE_FALSE_NEGATIVES, dict(sentence_id=criteria['sentence_id']))
            return dict(status=constant.STATUS_SENTENCE_MOVED + criteria['sentence_id'])

    async def sentence_context(self, criteria=None):
        if criteria['element_tag']=='img':
            return []
        sentence_hits = await self.dao.get(constant.DB_TABLE_REPORT_SENTENCE_HITS, dict(uid=criteria['uid']))
    ...
    async def confirmed_sentences(self, criteria=None):
        tmp = []
        techniques = await self.dao.get(constant.DB_TABLE_TRUE_POSITIVES,
                                        dict(sentence_id=criteria['sentence_id'], element_tag=criteria['element_tag']))
        for tech in techniques:
            name = await self.dao.get(constant.DB_TABLE_ATTACK, dict(uid=tech['uid']))
            tmp.append(name[0])
        return tmp
    ...
    async def true_positive(self, criteria=None):
        ...
        await self.dao.insert(constant.DB_TABLE_TRUE_POSITIVES, 
                              dict(sentence_id=sentence_dict[0]['uid'], 
                                   uid=criteria['attack_uid'],
    ...
    async def false_positive(self, criteria=None):
        ...
        await self.dao.insert(constant.DB_TABLE_FALSE_POSITIVES, 
                              dict(sentence_id=sentence_dict[0]['uid'], 
                                   uid=criteria['attack_uid'],
                                   false_positive=sentence_to_insert))
        return dict(status=constant.STATUS_INSERTED, last=last)
    ...
    async def insert_report(self, criteria=None):
        ...
        for i in range(len(criteria['title'])):
            ...
            temp_dict['id'] = await self.dao.insert(constant.DB_TABLE_REPORTS, temp_dict)
    ...
    async def start_analysis(self, criteria=None):
        ...
        regex_patterns = await self.dao.get(constant.DB_TABLE_REGEX_PATTERNS)
        ...
        await self.dao.update(constant.DB_TABLE_REPORTS, 'title', criteria['title'], dict(current_status='needs_review'))
        temp = await self.dao.get(constant.DB_TABLE_REPORTS,dict(title=criteria['title']))
        ...
        for sentence in analyzed_html:
            if sentence['ml_techniques_found']:
                ...
                await self.dao.insert(constant.DB_TABLE_REPORT_SENTENCES, data)

        for element in original_html:
            html_element = dict(report_uid=report_id, text=element['text'], tag=element['tag'], found_status="false")
            await self.dao.insert(constant.DB_TABLE_ORIGINAL_HTML, html_element)

    async def missing_technique(self, criteria=None):
        # Get the attack information for this attack id
        attack_dict = await self.dao.get(constant.DB_TABLE_ATTACK, dict(uid=criteria['attack_uid']))
        ...
        await self.dao.insert(constant.DB_TABLE_TRUE_POSITIVES, dict(sentence_id=sentence_dict[0]['uid'],
                                                                     uid=criteria['attack_uid'],
                                                                     true_positive=sentence_to_insert,
                                                                     element_tag=criteria['element_tag']))
        ...
        await self.dao.insert(constant.DB_TABLE_REPORT_SENTENCE_HITS, dict(uid=criteria['sentence_id'],
        ...
        if sentence_dict[0]['found_status'] == 'false':
            await self.dao.update(constant.DB_TABLE_REPORT_SENTENCES, 
                                  'uid', criteria['sentence_id'], dict(found_status='true'))
